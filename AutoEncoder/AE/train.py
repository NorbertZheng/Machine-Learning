...
# Initialize params, and then instantiate AE.
params_inst = params.default_params
ae_inst = models.AE(params_inst)
...
for epoch_idx in range(params_inst["n_epochs"]):
    ...
    for batch_idx in range(x_train.shape[0] // params_inst["batch_size"]):
        ...
        # Train the ae_inst using one-batch data.
        with tf.GradientTape() as gt:
            _, loss_i = ae_inst(X_i, training=True)
        # Modify weights to optimize the model.
        gradients_i = gt.gradient(loss_i, ae_inst.trainable_variables)
        gradients_i = [(tf.clip_by_norm(grad, 2), var)\
            if grad is not None else (grad, var)\
            for (grad, var) in zip(gradients_i, ae_inst.trainable_variables)]
...
z_sample = ae_inst.recognite(x_sample)
...
for i, yi in enumerate(x_values):
    for j, xi in enumerate(y_values):
        ...
        x_sample_i = ae_inst.generate(z_sample_i).numpy()
